...
/**
 * Add Login/Register Buttons Back - Minimal Professional Style
 * Only shows for anonymous users, hidden for logged-in users
 */

use Drupal\node\Entity\Node;

echo "🔐 ADDING LOGIN BUTTONS - MINIMAL STYLE\n";
echo "========================================\n\n";
...
// New HTML with login buttons for anonymous users
$html = <<<'HTML'
...
<style>
  * {
  ...
  .crm-login-banner {
    ...
    padding: 20px 24px;
    margin-bottom: 32px;
    display: none; /* Hidden by default, shown via JS for anonymous users */
    align-items: center;
...
<script>
  ...
  setTimeout(function() {
    ...
    if (!isLoggedIn && loginBanner) {
      ...
      console.log('Login banner shown for anonymous user');
    }
  }, 50);
...
echo "✅ Successfully added login buttons!\n\n";

echo "🔑 Login Banner Features:\n";
echo "   ✓ Minimal white card with border (not heavy gradient)\n";
echo "   ✓ Shows ONLY for anonymous users\n";
echo "   ✓ Hides automatically when logged in\n";
echo "   ✓ Clean design: 20px padding, subtle shadow\n";
echo "   ✓ Two buttons: Đăng nhập (blue) + Đăng ký (white)\n";
echo "   ✓ Responsive: stacks vertically on mobile\n\n";

echo "🎨 Design:\n";
echo "   • Background: white (not gradient)\n";
echo "   • Border: 1px solid #e2e8f0\n";
echo "   • Title: 18px, font-weight: 600\n";
echo "   • Subtitle: 14px, muted gray\n";
echo "   • Professional and clean\n\n";

echo "🌐 Test:\n";
echo "   Anonymous: http://open-crm.ddev.site/ (will see login banner)\n";
echo "   Logged in: Banner hidden automatically\n\n";
